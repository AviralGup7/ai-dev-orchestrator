...
const CASES = [
  ...
  },
  {
    ...
    file: 'src/core/environment.js',
    from: '    if (!failed && !s.conversationId) {',
  ...
  /* ---- session 11: the run that could not start -------------------- */
  {
    name: 'an unrequired surface blocks the bind again',
    file: 'src/core/environment.js',
    from: '    if (skipIfBroken && problems.length > problemsBefore) {',
    to: '    if (false) {',
    expect: 'UNREQUIRED SURFACE CANNOT BLOCK',
    test: 'test/environment.test.mjs',
  },
  {
    name: 'a required surface stops being validated',
    file: 'src/core/environment.js',
    from: '    const skipIfBroken = !isRequired;',
    to: '    const skipIfBroken = true;',
    expect: 'REQUIRED surface in the same broken state still refuses',
    test: 'test/environment.test.mjs',
  },
  {
    name: 'the worker regains a dynamic import()',
    file: 'extension/background.js',
    from: '        const snap = await snapshotEnvironment();\n        return verify(binding, snap);',
    to: "        const snap = await snapshotEnvironment();\n        const m = await import('../src/core/environment.js');\n        return m.verify(binding, snap);",
    expect: 'WORKER USES NO DYNAMIC import',
    test: 'test/packaging.test.mjs',
  },
  {
    name: 'runner events fall out of the log vocabulary again',
    file: 'src/core/bridge.js',
    from: "  'run-finished': 'workflow-completed',",
    to: '',
    expect: 'every event the runner emits has a place',
    test: 'test/packaging.test.mjs',
  },
  {
    name: 'a finished run stops reporting why',
    file: 'src/core/bridge.js',
    from: "      return e.verdict?.why || e.why || e.verdict?.reason || e.reason || 'Run ended';",
    to: "      return e.why || e.reason || 'Run ended';",
    expect: 'logs WHY it finished',
    test: 'test/packaging.test.mjs',
  },
  /* ---- session 12: long-running Arena tasks ------------------------ */
  {
    name: 'the engineer goes back to a four-minute budget',
    file: 'src/adapters/base.js',
    from: '    engineer: 4 * 3600_000,    // 4 HOURS: a build, a suite, a repository',
    to: '    engineer: 240_000,',
    expect: 'MULTI-HOUR ENGINEERING TASK IS NOT KILLED',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'the adapter ignores its per-surface budget',
    file: 'src/adapters/base.js',
    from: '          timeoutMs: this.policy.timeouts?.[this.surface] ?? this.policy.timeoutMs,',
    to: '          timeoutMs: this.policy.timeoutMs,',
    expect: 'sends its own per-surface budget',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'a frozen page is waited on for the whole budget',
    file: 'src/transports/dom.js',
    from: '      if (this.now() - lastChange > this.config.silenceMs) {',
    to: '      if (false) {',
    expect: 'SILENT page fails promptly',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'silence is not reset by real progress',
    file: 'src/transports/dom.js',
    from: '      if (s.busy || s.lastText !== lastText) lastChange = this.now();',
    to: '      /* never resets */',
    expect: 'slow but LIVE task keeps its budget',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'progress heartbeats stop being emitted',
    file: 'src/transports/dom.js',
    from: '      if (elapsed - reportedProgress >= 60_000) {',
    to: '      if (false) {',
    expect: 'progress is REPORTED during a long wait',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'elapsed is derived from the default budget again',
    file: 'src/transports/dom.js',
    from: '      const elapsed = this.now() - startedAt;',
    to: '      const elapsed = this.now() - (deadline - this.config.timeoutMs);',
    expect: 'progress is REPORTED during a long wait',
    test: 'test/transport.test.mjs',
  },
  {
  ...
  {
    name: 'raw page text is handed to the parser as a reply',
    file: 'src/transports/dom.js',
    from: "      via = 'none';",
    to: "      lastText = body; via = 'none';",
    expect: 'pageProbe reports page size',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'the fence fallback overrides a real selector match',
    file: 'src/transports/dom.js',
    from: '  if (!lastText) {',
    to: '  if (true) {',
    expect: 'pageProbe prefers a real selector match',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'a blank page and a blind page get the same message again',
    file: 'src/transports/dom.js',
    from: '        const blind = s.pageChars > 2_000;',
    to: '        const blind = true;',
    expect: 'a genuinely BLANK page is reported as blank',
    test: 'test/transport.test.mjs',
  },
  {
    /* The MV3 eviction bug: a dead run that renders as healthy. */
    name: 'the heartbeat goes back to being written and never read',
    file: 'src/core/session.js',
    from: "  if (run.state === 'running' && Number.isFinite(heartbeatAt)) {",
    to: '  if (false) {',
    expect: 'A RUN THAT SAYS RUNNING WITH A DEAD WORKER',
    test: 'test/session.test.mjs',
  },
  {
    name: 'every running run is called abandoned',
    file: 'src/core/session.js',
    from: '    if (since > HEARTBEAT_STALE_MS) {',
    to: '    if (true) {',
    expect: 'a LIVE run with a fresh heartbeat is not called abandoned',
    test: 'test/session.test.mjs',
  },
  {
    name: 'the abandonment window shrinks to a single missed beat',
    file: 'src/core/session.js',
    from: 'export const HEARTBEAT_STALE_MS = 90_000;',
    to: 'export const HEARTBEAT_STALE_MS = 25_000;',
    expect: 'the abandonment window tolerates several missed beats',
    test: 'test/session.test.mjs',
  },
  {
    name: 'a dead run keeps its green live dot',
    file: 'extension/ui.js',
    from: "  const dot = s.abandoned ? 'bad'",
    to: "  const dot = false ? 'bad'",
    expect: 'AN ABANDONED RUN LOSES THE LIVE DOT',
    test: 'test/ui.test.mjs',
  },
  {
    /* failedAttempts: declared, plumbed, and for the life of the project
       never written -- so the loop could not learn from a failure. */
    name: 'failedAttempts goes back to never being written',
    file: 'src/core/orchestrator.js',
    from: '    if (failed) {',
    to: '    if (false) {',
    expect: 'A FAILED ITERATION IS RECORDED',
    test: 'test/orchestrator.test.mjs',
  },
  {
    name: 'every iteration is recorded as a failure',
    file: 'src/core/orchestrator.js',
    from: "    const failed = result.outcome === 'partial'",
    to: "    const failed = true || result.outcome === 'partial'",
    expect: 'a SUCCESSFUL iteration is not recorded as a failed attempt',
    test: 'test/orchestrator.test.mjs',
  },
  {
    name: 'the failed-attempt list grows without bound',
    file: 'src/core/orchestrator.js',
    from: '      if (this.memory.failedAttempts.length > MAX_FAILED_ATTEMPTS) {',
    to: '      if (false) {',
    expect: 'the failed-attempt list is bounded',
    test: 'test/orchestrator.test.mjs',
  },
  {
    name: 'a score collapse is mislabelled as a plateau again',
    file: 'src/core/stop.js',
    from: '      if (delta <= -cfg.regressionDrop) {',
    to: '      if (false) {',
    expect: 'A COLLAPSE IS REPORTED AS A REGRESSION',
    test: 'test/stop-guard.test.mjs',
  },
  {
    name: 'ordinary scoring noise is called a regression',
    file: 'src/core/stop.js',
    from: '  regressionDrop: 5,',
    to: '  regressionDrop: 0.5,',
    expect: 'ordinary scoring noise is NOT called a regression',
    test: 'test/stop-guard.test.mjs',
  },
  {
    name: 'the rate-limit signal is detected and ignored again',
    file: 'src/core/surface.js',
    from: '  const limited = hit(RATE_LIMITED);',
    to: '  const limited = null;',
    expect: 'A USAGE LIMIT IS CLASSIFIED AS A REASON TO WAIT',
    test: 'test/surface.test.mjs',
  },
  {
    /* Run 202608081932: 60,433 characters of real work discarded because the
       parser wanted backticks that a RENDERED code block does not contain. */
    name: 'the parser requires backticks again, so rendered reports are lost',
    file: 'src/core/report.js',
    from: '  const at = src.lastIndexOf(fence);',
    to: '  const at = -1;',
    expect: 'A REPORT READ FROM A RENDERED PAGE IS FOUND',
    test: 'test/parse.test.mjs',
  },
  {
    name: 'brace matching is replaced by a lazy regex',
    file: 'src/core/report.js',
    from: '    const body = firstJsonObject(src, at + fence.length);',
    to: '    const body = (src.slice(at).match(/[{][^]*?[}]/) || [null])[0];',
    expect: 'BRACES INSIDE PROSE DO NOT TRUNCATE THE REPORT',
    test: 'test/parse.test.mjs',
  },
  {
    name: 'the paste claims success without reading the text back',
    file: 'extension/dom-page.js',
    from: '  if (got.length < Math.min(text.length, 32)) {',
    to: '  if (false) {',
    expect: 'A COMPOSER THAT SILENTLY REJECTS THE TEXT IS A FAILED PASTE',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'a send button that is not mounted yet ends the iteration again',
    file: 'extension/dom-page.js',
    from: "    if (which === 'send' && composer) {",
    to: '    if (false) {',
    expect: 'A SEND BUTTON THAT IS NOT MOUNTED YET FALLS BACK TO ENTER',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'the Enter fallback is used for stop, which would submit instead',
    file: 'extension/dom-page.js',
    from: "    if (which === 'send' && composer) {",
    to: '    if (composer) {',
    expect: 'a missing STOP control does NOT fall back to Enter',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'run-failed is aliased onto iteration-failed again',
    file: 'src/core/bridge.js',
    from: "  'run-failed': 'workflow-failed',",
    to: "  'run-failed': 'iteration-failed',",
    expect: 'A RUN-LEVEL FAILURE NEVER RENDERS AS',
    test: 'test/ui.test.mjs',
  },
  {
    /* Run 202608090550: ChatGPT's ProseMirror composer never received the
       text, so the message was never sent and the run waited out the budget. */
    name: 'the composer is written via the DOM again, which ProseMirror ignores',
    file: 'extension/dom-page.js',
    from: "      inserted = document.execCommand('insertText', false, text);",
    to: '      inserted = false;',
    expect: 'THE COMPOSER IS FILLED VIA execCommand',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'the fallback drops beforeinput, so the editor ignores the input',
    file: 'extension/dom-page.js',
    from: "      for (const type of ['beforeinput', 'input']) {",
    to: "      for (const type of ['input']) {",
    expect: 'the DOM fallback fires a spec-shaped beforeinput',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'the insert appends instead of replacing, double-sending on retry',
    file: 'extension/dom-page.js',
    from: '      range.selectNodeContents(el);',
    to: '      /* no selection */',
    expect: 'the insert REPLACES existing content',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'the paste stops checking whether the framework enabled send',
    file: 'extension/dom-page.js',
    from: '    enabledSend = send',
    to: '    enabledSend = true || send',
    expect: 'the paste reports whether the FRAMEWORK enabled the send control',
    test: 'test/transport.test.mjs',
  },
  {
    name: 'submit assumes the click worked instead of verifying it',
    file: 'extension/dom-page.js',
    from: "  if (await composerEmptied(composer)) return { ok: true, via: 'click' };",
    to: "  if (true) return { ok: true, via: 'click' };",
    expect: 'A CLICK THE PAGE IGNORES IS A FAILED SUBMIT',
    test: 'test/transport.test.mjs',
  },
  {
  ...
  {
    /* The evaluator scored `testing` as MEASURED off an English sentence. */
    name: 'the mocha pattern matches prose again',
    file: 'src/core/parse.js',
    from: "    re: /^[ \\t]*(\\d[\\d,_]*)\\s+passing\\b[ \\t]*(?:\\([^)]*\\))?[ \\t]*$/im,",
    to: '    re: /(\\d[\\d,_]*)\\s+passing/i,',
    expect: 'A SENTENCE MENTIONING "PASSING" IS NOT SCRAPED',
    test: 'test/parse.test.mjs',
  },
  {
    name: 'per-line backticks are no longer unwrapped',
    file: 'src/core/report.js',
    from: "  if (!text.includes('`')) return text;",
    to: '  return text;',
    expect: 'A REPORT WRAPPED IN PER-LINE BACKTICKS PARSES',
    test: 'test/parse.test.mjs',
  },
  {
    name: 'the backtick unwrap eats markdown fences',
    file: 'src/core/report.js',
    from: "  return text.replace(/^([ \\t]*)`([^`].*?)`[ \\t]*$/gm, '$1$2');",
    to: "  return text.replace(/^([ \\t]*)`(.*)`[ \\t]*$/gm, '$1$2');",
    expect: 'the fenced form still wins when both are present',
    test: 'test/parse.test.mjs',
  },
  {
    name: 'an identical reply is retried anyway',
    file: 'src/adapters/base.js',
    from: '      if (previousText !== null && text === previousText) {',
    to: '      if (false) {',
    expect: 'A BYTE-IDENTICAL REPLY SHORT-CIRCUITS',
    test: 'test/integration.test.mjs',
  },
  {
    name: 'the short circuit fires on every retry, killing self-correction',
    file: 'src/adapters/base.js',
    from: '      if (previousText !== null && text === previousText) {',
    to: '      if (previousText !== null) {',
    expect: 'a model that CORRECTS itself on retry still succeeds',
    test: 'test/integration.test.mjs',
  },
  {
  ...
  },
  {
    ...
    to: '  /* document order */',
    expect: 'ranks a composer above sidebar chrome',
    test: 'test/surface.test.mjs',
  },
  {
    name: 'the depth cap excludes a real composer again',
    file: 'extension/scan.js',
    from: 'maxDepth = 40, maxText = 400 } = {}) {',
    to: 'maxDepth = 12, maxText = 400 } = {}) {',
    expect: 'deep composer is not excluded',
    test: 'test/surface.test.mjs',
  },
];
